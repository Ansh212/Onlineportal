...
print(df[FEATURE_COLUMNS].isnull().sum().loc[lambda x : x > 0]) # Show only columns with NaNs before filling
...
if not all(item in [0, 1] for item in y.unique()):
    print("Warning: Target column contains values other than 0 and 1. Check extract_features.py labeling.")
...
coeffs = pd.Series(model.coef_[0], index=FEATURE_COLUMNS)
